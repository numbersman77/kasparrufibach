...
$$
\hat \theta \ = \ \exp\Bigl(-q_{1 - \alpha / 2} \text{SE}(\hat{\theta})\Bigr).
$$
Let us verify this:
...
\theta \ = \ \pm(q_{1 - \alpha / 2} + q_{1 - \beta}) \sqrt{4 / d}.
$$
So:
...
# exponentiate to get hazard ratio (theta = log(HR)):
fac <- qnorm(1 - alpha / 2) + qnorm(1 - beta2)
exp(c(-1, 1) * fac * sqrt(4 / nevent))
```
## Using rpact
...
```{r, include=TRUE, echo=TRUE}
nevent0$criticalValuesEffectScaleLower
```
...
- Since the critical value of a hypothesis test is computed assuming the null hypothesis $H_0$ is true the above consideration does not tell us anything about for which effect size the hypothesis test is statistically significant. We call this effect the **minimal detectable difference**. Above I have illustrated several ways to compute this MDD and in our example it amounts to `r round(mdd, 3)`.

Now, to me it somewhat surprising that in clinical trial design focus is so much on the `r colFmt("effect we power at")`, i.e. the hazard ratio of `r hr` in our example. In my opinion this carries a risk of being misinterpreted in the sense that stakeholders are of the opinion that - if the trial is statistically significant - we will indeed observe a HR of `r hr`. However, that is obviously not the case: the trial will also be statistically significant for any final HR estimate in the interval $(`r hr`, `r round(mdd, 3)`]$. So, when designing a trial planners should not focus on a discussion of the HR we power at, but rather on the MDD and ask
