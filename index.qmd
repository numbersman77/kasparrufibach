---
title: ""
output: 
  rmarkdown::html_document:
    highlight: pygments
    number_sections: no
    self_contained: yes
    toc: no
    toc_depth: 2
    toc_float: yes
---

```{r setup, include = FALSE, echo = FALSE}
## load packages
packs.html <- c("readxl")
for (i in 1:length(packs.html)){library(packs.html[i], character.only = TRUE)}

path <- getwd()
up <- as.data.frame(read_excel(paste(path, "/data/update.xlsx", sep = ""), col_types = NULL))[1, 1]
```

::: {.panel-tabset .nav-pills}

# About

<style type="text/css">
.tg  {border-collapse:collapse;border-spacing:0;}
.tg td{border-color:white;border-style:solid;border-width:5px;font-family:Arial, sans-serif;font-size:14px;
  overflow:hidden;padding:10px 10px;word-break:normal;}
.tg th{border-color:whitek;border-style:solid;border-width:5px;font-family:Arial, sans-serif;font-size:14px;
  font-weight:normal;overflow:hidden;padding:10px 10px;word-break:normal;}
.tg .tg-0lax{text-align:left;vertical-align:top}
</style>
<table class="tg">
<thead>
  <tr>
    <td class="tg-0lax">![](img/kaspar.jpg){width=200}</td>
    <td class="tg-0lax">
    <br><br>
    <a href="https://linkedin.com/in/kasparrufibach" class="btn btn-info" target = "frame"><i class="fa fa-download"></i> Linkedin</a> 
    <a href="https://scholar.google.com/citations?user=SM74TkcAAAAJ&hl=de" class="btn btn-info" target = "frame"><i class="fa fa-download"></i> Google scholar</a>
    <a href="https://orcid.org/0000-0002-2634-1167" class="btn btn-info" target = "frame"><i class="fa fa-download"></i> orcid</a>
    <a href="https://arxiv.org/search/?query=rufibach%2C+k&searchtype=all&abstracts=hide&order=-announced_date_first&size=50" class="btn btn-info" target = "frame"><i class="fa fa-download"></i> arxiv</a>
    <a href="https://github.com/numbersman77" class="btn btn-info" target = "frame"><i class="fa fa-download"></i> github</a>
    <br><br>
    Last update of this page: `r up`.
    </td>
  </tr>
</thead>
</table>

I have a strong biostatistical background and a deep understanding of clinical research, regulatory environments, and the product development lifecycle within life sciences. I am passionate about applying this expertise to solve problems in pharmaceutical drug development.

<a href="files/01_Kaspar_Rufibach_CV.pdf" class="btn btn-info"><i class="fa fa-download"></i> CV</a>
<a href="files/02_Kaspar_Rufibach_publications.pdf" class="btn btn-info"><i class="fa fa-download"></i> Publications</a>
<a href="files/03_Kaspar_Rufibach_talks.pdf" class="btn btn-info"><i class="fa fa-download"></i> Talks</a>
<a href="files/04_Kaspar_Rufibach_teaching.pdf" class="btn btn-info"><i class="fa fa-download"></i> Teaching</a>

I am an [Accredited European Statistician](files/Accreditation_FENStat.pdf), a qualification issued by [The Federation of European National Statistical Societies](https://fenstats.eu/).

I am a member of the [ICH E20 expert writing group](https://www.ich.org/page/efficacy-guidelines#19-1) as a deputy topic lead, representing [EFPIA](https://www.efpia.eu/).

# What I believe in

I see me supporting statisticians and drug developers to continuously challenge the status quo, with the aim of improving the drug development process, making it more efficient, and enabling access. To achieve this, I believe that an attitude and mindset of

- integrity, 
- teamwork,
- scientific rigour,
- solid understanding of statistical methodology, 
- a genuine desire to collaborate beyond organizational boundaries (both within and across companies, with regulators, and with academics), 
- and an insatiable curiosity

are essential. 

# Key achievements 

I have a reputation of getting things done.

Key achievements in my career:

- Built global network among statisticians in drug development, i.e. industry, regulators, and academia.
- Biostatistics or biometrics lead for filings of oncology and fertility drugs. 
- Designed, ran, analyzed, and discussed with regulators various early and late phase clinical trials.
- At both Roche and Merck, co-developed and rolled-out quantitative decision-making framework in R&D.
- Co-founded and co-lead X-industry working groups with global impact: [Oncology estimand WG](http://www.oncoestimand.org/), [EFSPI statistical methodology leaders group](https://efspieurope.github.io/efspi/methods/methods_intro.html), [SAVVY](https://numbersman77.github.io/savvy).
- Chair (3x) and member of Scientific Committee (since 2017) of organizing committee of [EFSPI regulatory statistics workshop](http://www.efspiworkshop.org/).
- Co-chair of [ISCB46 2026 in Basel](https://iscb2025.info/) (global statistics conference with >850 attendees).
- Member of [ICH E20 expert writing group](https://www.ich.org/page/efficacy-guidelines#19-1).
- Giving many courses for statisticians and non-statisticians on many levels.
- Initiated and teaching regular course series on statistical and quantitative topics for non-statisticians with 100s of regular attendees at Roche and Merck. 
- Supervised or co-supervised one PhD and ten MSc thesis.
- More than 100 scientific papers, many of them in top statistical or medical journals.
- Written or co-written 15 R packages on CRAN.

# Interests

- Statistical methods to optimize clinical trial designs
- Advanced survival analysis
- Probability of success
- Estimands and causal inference
- Nonparametric statistics

# Education

- Postdoc in Mathematical Statistics, 2007, [Stanford University](https://statistics.stanford.edu/), United States
- PhD in Mathematics ([dissertation](https://biblio.unibe.ch/download/eldiss/06rufibach_k.pdf)), 2006, [University of Bern](https://www.imsv.unibe.ch/), Switzerland
- MSc in Mathematical Statistics and Actuarial Science, 2001, [University of Bern](https://www.imsv.unibe.ch/), Switzerland

# Short bio

Kaspar Rufibach is a biostatistician who is passionate about supporting statisticians and drug developers to continuously challenge the status quo, with the aim of improving the drug development process, making it more efficient, and enabling access. 

Kaspar has co-founded and co-leads the special interest group [Estimands in oncology](http://www.oncoestimand.org) which has (as of August 2024) more than 100 members from 50 institutions globally. He has also co-founded and co-leads the [EFSPI statistical methodology leader group](https://efspieurope.github.io/efspi/methods/methods_intro.html) which has 16 members from 16 companies. He regularly interacts with regulators globally on various joint projects.

Kaspar’s research interests are methods to optimize study designs, platform trials, advanced survival analysis, probability of success, estimands and causal inference, and estimation of treatment effects in subgroups.

Kaspar received training and worked as a statistician at the Universities of [Bern](https://www.imsv.unibe.ch/), [Stanford](https://statistics.stanford.edu/), and [Zurich](https://www.uzh.ch). During his PhD he gained some first experience in clinical research as a part-time statistician at the [Swiss Institute for Applied Cancer Research](https://www.sakk.ch/en). From 2012 until 2024 he worked at [Roche](https://www.roche.com) in Basel before joining [Merck KGaA](https://www.merckgroup.com/en) in October 2024 (100% home-based) to co-lead its Advanced Biostatistical Science group.

:::